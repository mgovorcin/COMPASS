...
def _constant_az_lut(radar_grid, offset):
    ...
    the model correction LUTs are disabled but an azimuth re-registration
    offset is still requested.
    '''
...
def run(cfg: GeoRunConfig):
    ...
    for burst_id, bursts in bursts_grouping_generator(cfg.bursts):
        ...
        if cfg.lut_params.enabled:
            ...
            # Even with the model corrections off, honour a requested azimuth
            # offset by feeding geocode_slc a constant azimuth-time LUT.
            az_lut = _constant_az_lut(burst.as_isce3_radargrid(),
                                      az_time_offset) \
